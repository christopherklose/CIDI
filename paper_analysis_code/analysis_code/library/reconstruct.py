...
def focus(holo, roi, mask=1,phase=0, prop_dist=0,dx=0, dy=0, scale=(0,100), experimental_setup={'ccd_dist':18e-2, 'energy':779.5, 'px_size':20e-6}, max_prop_dist=10, colorscale= (1,99)):
    '''
    ...
    def p(x, y, fx, fy):
        ...
        mi,ma=np.percentile(np.abs(simage_mask), (colorscale[0],colorscale[1]))
        ax1 = axs[0,0].imshow(np.abs(simage), vmin=mi, vmax=ma)
        axs[0,0].set_title("Abs")
        
        mi,ma=np.percentile(np.angle(simage_mask), (colorscale[0],colorscale[1]))
        ax2 = axs[0,1].imshow(np.angle(simage), cmap='twilight', vmin=mi, vmax=ma)
        axs[0,1].set_title("Phase")
        
        mi,ma=np.percentile(np.real(simage_mask), (colorscale[0],colorscale[1]))
        ax3 = axs[1,0].imshow(np.real(simage), cmap='gray', vmin=mi, vmax=ma)
        axs[1,0].set_title("Real Part")
        
        mi,ma=np.percentile(np.imag(simage_mask), (colorscale[0],colorscale[1]))
...
def focusCDI(pos,neg, roi, mask=1,phase=0, prop_dist=0,dx=0, dy=0, scale=(0,100), experimental_setup={'ccd_dist':18e-2, 'energy':779.5, 'px_size':20e-6}, operation="-", max_prop_dist=10):
    ...
    slider_phase = widgets.FloatSlider(min=-np.pi, max=np.pi, step=0.001, value=phase, layout=layout,
                                       description='phase shift', style=style)
    slider_dx = widgets.FloatSlider(min = -7, max = 7, step = 0.01, value = dx, layout = layout,
                                       description = 'x shift', style = style)
    slider_dy = widgets.FloatSlider(min = -7, max = 7, step = 0.01, value = dy, layout = layout,
                                       description = 'y shift', style = style)
