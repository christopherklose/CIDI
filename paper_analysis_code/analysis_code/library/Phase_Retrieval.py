...
def ShrinkWrap(inv,mask,SW_sigma=1, SW_thr=0.01):
    ...
    mask_labeled,nb_labels=label(mask>=1)
    mask_new=cp.zeros(mask.shape)
    ...
    if labeled:
        for i in np.arange(1,nb_labels+1):
            mask_new+=1*(mask_labeled==i)*(blurred>=(SW_thr*cp.amax(blurred[mask_labeled==i])))
    else:
        mask_new=1*(blurred>=(SW_thr*cp.amax(blurred[mask==1])))
...
def RL(Idelta, Iexp, gamma_cp, RL_it, smth_func=1):
    ...
    for l in range(RL_it):
        ...
        Denom=((np.fft.ifft2((cp.fft.fft2(gamma_cp)*Id))).real)
        ...
        gamma_cp = cp.abs(gamma_cp*cp.fft.ifft2(Id_1*(cp.fft.fft2(Iexp/Denom))))
