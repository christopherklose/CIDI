...
def load_single_RB(image, topo, helicity, auto_factor=False):
    '''
...
def set_center_array(image, center):
    '''
    Move given coordinate to center of image. for array of images
    Rolls input image to new center.
    ...
    im_roll = np.roll(image, (0,dx, dy), axis=(0, 2, 1))
...
def mask_beamstop(holo, bs_size, sigma = 3, center = None):
    ...
    #create the beamstop mask using scikit-image's disk function
    bs_mask = np.zeros(holo.shape)
    yy, xx = disk((y0, x0), bs_size/2)
    bs_mask[yy, xx] = 1
