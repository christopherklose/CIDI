...
for ii in np.arange(n):
    ...
    pattern=1*fluctuation+1*image1*(np.exp(0.0j*(phase1)))
...
pg.image(np.abs(retrieved_res0)**2,title='diffraction pattern retrieved from CIDI')
pg.image(np.abs(F_stochastic_numerical),title='diffraction pattern from numerical data')
pg.image(np.abs(ret_pattern),title='Absolute value of retrieved image (object plane) from CIDI')
pg.image(np.angle(ret_pattern),title='Phase of retrieved image (object plane) from CIDI')
